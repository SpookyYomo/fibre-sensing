...
# phase plotting
def phase_reconstruction_naive(phases, tol):
    ...
    Inputs
    ----
    phase (np iterable): phase generated from signal_to_phase functions
    ...
    for i in range(1,len(phases)):
        if phases[i] - phases[i-1] > tol:
            r[i:] = r[i:] - np.array([2*pi]*(len(phases)-i))
        elif phases[i] - phases[i-1] < -tol:
            r[i:] = r[i:] + np.array([2*pi]*(len(phases)-i))
    
    return r

def phase_reconstruction(phases, tol, phase_advance = 0):
    ...
    Inputs
    ----
    phase (iterable): phase generated from signal_to_phase functions
    tol (float): if change in phase exceeds this value, perform a wrap
    ...
    r = [phases[0]]
    mod_twopi = 0
    phase_advance = phase_advance % (2*pi)
    for i in range(1, len(phases)):
        if phases[i] - phases[i-1] > tol:
            mod_twopi -= 1
        elif phases[i] - phases[i-1] < -tol:
            mod_twopi += 1
        r.append(phases[i] + 2 * pi * mod_twopi - phase_advance)
    return np.asarray(r)
...
def signal_to_phase(signal, N, dphi, phase_advancement_correction=True):
    ...
    Qs = (2/N) * np.fromiter([np.dot(signal[j:j+N], cosines) for j in range(len_signal-N)], 
        dtype=np.float64)
    print(f"[Debug] Expected I[0] = {(2/N)*np.sum([signal[0+i]*sines[i] for i in range(N)])}")
    print(f"[Debug] {Is[0] = }")
    print(f"[Debug] {Qs[0] = }")
