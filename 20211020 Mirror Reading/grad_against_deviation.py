...
folderpath = os.path.dirname(__file__)+'\Temp Batch'
files = listdir(path=folderpath)

# This file's specific function
def get_f_A_and_deviation_from_filename(txt):
    Hz_pos = txt.find('Hz')
    mVpp_pos = txt.find('mVp')
    delta_pos_after = txt.find('Hz00')
    delta_pos_before = mVpp_pos + 5
    f = int(txt[Hz_pos-3:Hz_pos])
    A = int(txt[mVpp_pos-3:mVpp_pos])
    delta = int(txt[delta_pos_before:delta_pos_after])
    return f, A, delta
...
def main():
    ...
    for file in files:
        ...
        if INDIVIDUAL_PLOTS:
            ...
            # plt.title(f"$f_{{Mirror}}$ = {f}Hz, Piezo Ampl = {mVpp}mVpp, $f$ = {float('80.1250'+str(delta_corr))}MHz, \n(A, $f$, $\phi_0$, $m$, c) = {fittings}", useTex = True )
            plt.title(f"$f_{{Mirror}}$ = {f}Hz, Piezo Ampl = {mVpp}mVpp, $\Delta$ = {delta_corr}Hz, \n(A, $f$, $\phi_0$, $m$, c) = {fittings}", useTex = True )
